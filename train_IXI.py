...
def main():
    parser = argparse.ArgumentParser(description='IXI')
    ...
    train_dataset = IXIDataset(args.dataset, mode='train')
    valid_dataset = IXIDataset(args.dataset, mode='valid')    
    ...
    model = HFGAN(dim=args.ch_dim, num_inputs=3, num_outputs=1, dim_mults=(1,2,4,8,10), n_layers=args.numlayers, skip=True, blocks=False, image_size=256)
    discriminator = Discriminator(channels=1, num_filters_last=args.ch_dim, ixi=True)

    optimizer = Adam(model.parameters(), lr=0.0)
    optimizer_D = Adam(discriminator.parameters(), lr=0.0)
    steps_per_epoch = len(train_loader)
    total_iteration = epochs*steps_per_epoch*3
    ...
    if args.resume:
        ...
        epoch = scheduler.scheduler.T_cur // steps_per_epoch / 3
    ...
    while epoch < epochs:
        ...
        for n, batch in enumerate(train_loader):
            with accelerator.accumulate(model):
                ...
                for m_shift in [False, True, True]:
                    iterations += 1
                    if m_shift:
                        for idx in range(inputs_all.shape[0]):
                            modalities[idx] = modalities[idx]+1 if modalities[idx]!=2 else 0
        ...
        if epoch % valid_epochs == 0:
            ...
            with torch.no_grad():
                ...
                for batch in valid_loader:
                    ...
                    input_modals = []
                    for n, m in enumerate(modalities):
                        input_modals.append([x for x in [0,1,2] if x != m])
